      ...
      <Card className="p-4 bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 backdrop-blur-sm shadow-md">
        ...
        <div className="grid grid-cols-7 gap-1 text-center text-xs mb-2">
          {dayNames[lang].map((day, idx) => (
            <div
              key={idx}
              className={`font-medium ${idx === 0 ? "text-red-500 dark:text-red-400" : idx === 6 ? "text-blue-500 dark:text-blue-400" : "text-gray-700 dark:text-slate-300"}`}
            >
              {day}
            </div>
          ))}
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-lg text-gray-900">
                {selectedDate?.toLocaleDateString(
                  language === "ko" ? "ko-KR" : language === "en" ? "en-US" : language === "zh" ? "zh-CN" : "ja-JP",
                )}
              </h3>
              <Button variant="ghost" size="sm" onClick={() => setShowEventsModal(false)}>
                <X className="h-4 w-4" />
              </Button>
            </div>
