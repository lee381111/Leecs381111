...
export async function POST(request: Request) {
  try {
    ...
    const systemPrompt =
      language === "ko"
        ? `당신은 친절한 AI 비서입니다. 사용자의 일정, 노트, 차량 관리, 할일, 건강 기록 등을 도와주는 개인 비서 역할을 합니다. 간결하고 명확하게 답변하세요. 반드시 한국어로 답변하세요.${userContext}`
        : language === "en"
          ? `You are a friendly AI assistant. You help users manage their schedules, notes, vehicle maintenance, todos, health records, and more. Provide concise and clear responses. IMPORTANT: Always respond in English only.${userContext}`
          : language === "zh"
            ? `您是一位友好的AI助手。您帮助用户管理日程、笔记、车辆维护、待办事项、健康记录等。请提供简洁明了的回答。重要：请仅用中文回答。${userContext}`
            : `あなたは親切なAIアシスタントです。ユーザーのスケジュール、ノート、車両管理、やること、健康記録などをサポートします。簡潔で明確な回答を提供してください。重要：必ず日本語で回答してください。${userContext}`
